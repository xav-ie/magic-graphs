<script setup lang="ts">
	...
	import { useProductRouting, getCurrentProduct, isExternal } from '@utils/product';
...
<template>
	<div @mouseenter="hovered = true" @mouseleave="hovered = false" class="relative">
		<div
		...
		>
			<div v-if="currentProduct.productId !== product.productId" class="flex items-center gap-3">
				<GButton
				...
				>
					...
					<img src="/icons/arrow_right_alt.svg" class="w-8 h-8" alt="go to product" />
					go
				</GButton>
				<GButton
					...
					class="grid place-items-center w-[120px] text-sm"
				>
					<img src="/icons/step_over.svg" class="w-8 h-8" alt="go to product with graph" />
					go with graph
				</GButton>
			</div>
			<GWell
