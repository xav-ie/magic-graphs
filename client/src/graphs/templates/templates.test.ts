...
import { getAverageCoordinatesOfGraphNodes,generateTemplateFromOriginCoordinates } from './helpers';

describe('getAverageCoordinatesOfGraphNodes', () => {
  it('should return the average coordinates for a list of nodes', () => {
    const nodes = [{ x: 10, y: 20 }, { x: 30, y: 40 }, { x: 50, y: 60 }];
...
describe('generateTemplateFromOriginCoordinates', () => {
  it('should center nodes around the target origin (0, 0)', () => {
    ...
    const result = generateTemplateFromOriginCoordinates(nodes, targetOrigin);
  ...
  it('should not mutate the original node array', () => {
    ...
    generateTemplateFromOriginCoordinates(nodes, targetOrigin);
