...
const getSteadyStateVector = (transitionMatrix: TransitionMatrix) => {
  ...
  const { data: augmentedMatrix } = identity.minus(inputMatrix) as {
  ...
  const bigNumberMatrix = augmentedMatrix.map((row) =>
    row.map((entry) => new BigNumber(entry)),
  );
  const solvedBigNumberMatrix = rref(bigNumberMatrix);
  const solvedMatrix = solvedBigNumberMatrix.map((row) =>
    row.map((entry) => entry.toNumber()),
  );

  return new Matrix(solvedMatrix).trans().data.at(-1).slice(0, -1) as number[];
};
...
export const useMarkovSteadyState = (
  graph: Graph,
  options: MarkovSteadyStateOptions,
) => {
  return computed(() => {
